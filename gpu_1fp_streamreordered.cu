...
int main(int argc, char **argv) {
    ...
    int first = 1;
    for(sector_id=0; sector_id < 126; sector_id++) {

        if (!first) {
            int prev_stream_id = (sector_id-1) % NSTREAMS;
            int prev_result_offset = prev_stream_id * (m/2)*RESULT_SIZE;

            cudaMemcpyAsync(&result[prev_result_offset], &d_result[prev_result_offset], (m/2)*RESULT_SIZE*sizeof(float), cudaMemcpyDeviceToHost, stream[prev_stream_id]);
            first = 0;
        }
        ...
        for (int i=0; i<m; i++) {
            for (int j=0; j<n; j++) {
                cin >> a >> b;
                iqhh[next_offset+i*n+j] = make_cuFloatComplex(a, b);
            }
        }
        for (int i=0; i<m; i++) {
            for (int j=0; j<n; j++) {
                cin >> a >> b;
                iqvv[next_offset+i*n+j] = make_cuFloatComplex(a, b);
            }
        }
